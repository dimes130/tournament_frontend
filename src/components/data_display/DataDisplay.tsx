...
export function DisplayTeams({ team }: DisplayTeamProp){
    ...
    const handleClick = () => {
        navigate(`/coach/${coach_id}/teams/${team.team_id}/plays`); // Change to your route path
      };
...
<div className="stat">
  ...
  <div className="stat">
    <button className="btn btn-primary btn-md py-10" onClick={handleClick}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
...
export function DisplayTeamDetails( { play } : DisplayPlayProp){
  const navigate = useNavigate();
  const {coach_id, team_id} = useParams();

  const handleClick = () => {
      navigate(`/coach/${coach_id}/teams/${team_id}/plays/create/${play.play_id}`);
  };
